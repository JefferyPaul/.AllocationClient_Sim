...
def main():
    for strategy_name in os.listdir(PATH_INPUT_PM):
        ...
        for file_name in os.listdir(path_strategy_pm):
            ...
            # 读取文件
            s_least_dt_pm = datetime.datetime.strftime(datetime.datetime.today() - datetime.timedelta(days=1), '%Y%m%d')
                ...
                while True:
                    ...
                    if line_sim == '':
                        ...
                        if line_sim.split(',')[0] == 'TraderId':
                            line_new_from_sim = ''
                            break
                        if line_sim.split(',')[1] > s_least_dt_pm:
                            line_new_from_sim = line_sim
                            break
                        else:
                            line_new_from_sim = ''
                            print('Not find new sim data, ', strategy_name, file_name)
                            break
...
if __name__ == '__main__':
    ...
    PATH_INPUT_SIM = os.path.join(os.path.dirname(PATH_ROOT), '1.SimulationData', '3.Output')
    PATH_INPUT_PM = os.path.join(os.path.dirname(PATH_ROOT), '2.PMData', 'Data')
